<x-app-layout>
    <x-slot name="title">Tambah Kategori | PharmaCare</x-slot>

    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 px-4">
        <div>
            <h1 class="text-4xl font-black text-gray-900 tracking-tight">Tambah Kategori</h1>
            <p class="text-blue-600/60 font-medium mt-1">Lengkapi detail kategori untuk pengelompokan obat yang lebih rapi.</p>
        </div>
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="w-full px-4">
        @csrf
                    ...
                    <div class="space-y-10">
                        <div>
                            ...
                            <input type="text" name="name" id="name" onkeyup="generateSlug()" 
                                ...
                                placeholder="Misal: Obat Sirup Anak" required>
                            @error('name')
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="5" 
                                ...
                                placeholder="Berikan penjelasan singkat mengenai kategori ini..."></textarea>
                                @error('description')
                                    <span class="text-red-600">{{ $message }}</span>
                                @enderror
                        </div>
                                ...
                                <label for="icon" class="relative flex flex-col items-center justify-center w-full h-44 border-2 border-blue-100 border-dashed rounded-[2.5rem] cursor-pointer bg-blue-50/20 hover:bg-blue-500/5 hover:border-blue-400 transition-all group overflow-hidden shadow-sm">
                                    <div id="dropzone-content" class="flex flex-col items-center justify-center pt-5 pb-6">
                                        <div class="w-14 h-14 mb-4 bg-white rounded-2xl flex items-center justify-center shadow-md group-hover:scale-110 transition-transform group-hover:shadow-blue-200">
                                            <svg class="w-7 h-7 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"></path></svg>
                                        </div>
