<x-app-layout>
    <x-slot name="title">Edit Produk Baru | PharmaCare</x-slot>

    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 px-4">
        <div>
            <h1 class="text-4xl font-black text-gray-900 tracking-tight">Edit Produk</h1>
            <p class="text-teal-600/60 font-medium mt-1">Edit detail obat atau produk kesehatan baru ke dalam katalog.</p>
        </div>
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="w-full px-4 pb-20">
        @csrf @method('PUT')
                    ...
                    <div class="space-y-10">
                        <div>
                            ...
                            <input type="text" name="name" id="name" onkeyup="generateSlug()" 
                                ...
                                placeholder="Contoh: Paracetamol 500mg" required value="{{ old('name', $product->name) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <input type="number" name="price" id="price" 
                                    ...
                                    placeholder="0" required value="{{ old('price', $product->price) }}">
                        ...
                        <div>
                            ...
                            <textarea name="about" id="about" rows="5" 
                                ...
                                placeholder="Jelaskan kegunaan, dosis, atau deskripsi produk...">{{ old('about', $product->about) }}</textarea>
                    ...
                    <div class="space-y-10">
                        ...
                        <div>
                            ...
                            <label for="image" class="relative flex flex-col items-center justify-center w-full h-[320px] border-2 border-teal-100 border-dashed rounded-[3rem] cursor-pointer bg-teal-50/20 hover:bg-teal-500/5 hover:border-teal-400 transition-all group overflow-hidden shadow-sm">
                                <div id="dropzone-content" class="flex flex-col items-center justify-center pt-5 pb-6" {{ $hasImage ? 'opacity-0' : '' }}>
                                ...
                                <img id="image-preview" 
                                        src="{{ $hasImage ? asset('storage/' . $product->image) : '' }}" 
                                        class="absolute inset-0 w-full h-full object-cover rounded-[2.5rem] {{ $hasImage ? '' : 'hidden' }}" />
